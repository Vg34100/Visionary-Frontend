...
export default function Index() {
  ...
  async function takePhoto() {
    if (cameraRef.current) {
      try {
        ...
        await uploadImageToServer(croppedPhoto.uri);
      } catch (error) {
  ...
  async function uploadImageToServer(imageUri: string) {
    ...
    try {
      const response = await fetch('http://192.168.205.90:8000/ocr/upload/', {
        method: 'POST',
        headers: {
